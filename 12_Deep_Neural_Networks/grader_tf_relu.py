...
def get_result(output):
  ...
  answer = np.array([[5.11000013, 8.44000053], [0., 0.],
  ...
  if output_shape != answer_shape:
    ...
    result[
      'comment'] = f'Output shape is {output_shape}, answer shape is {answer_shape})'
...
def run_grader(output):
  ...
  if not np.any(output):
    print(
      "Don't forget to complete all tasks and name your session variable output"
    )
